...
if (isset($_POST['login'])) {
    if (empty(trim($_POST['email']))) {
        $invalidInputEmail = "E-Mail-Feld ist erforderlich";
    }

    if (empty(trim($_POST['password']))) {
        $invalidInputPassword = "Passwortfeld ist erforderlich";
    }
    ...
    if (!empty(trim($_POST['email'])) && !empty(trim($_POST['password']))) {
        ...
        header("Location:login.php?err_msg=Kein Benutzer mit diesen Informationen gefunden");
        exit();
    }
}
?>
...
<body class="auth">
    <main class="form-signin w-100 m-auto">
        <form method="post">
            <div class="fs-2 fw-bold text-center mb-4">PAYAM.de</div>
